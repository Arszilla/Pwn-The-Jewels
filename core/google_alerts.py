    ...
    async def remove_alert(self, url):
        async with aiosqlite.connect(constants.Database.name) as database:
            ...
            # If the URL exists in the database:
                ...
                async with database.execute("SELECT keyword FROM google_alerts_links WHERE url=?",
                                        (url,)) as keyword_cursor:
                    result = await keyword_cursor.fetchall()

                keyword = [keyword[0] for keyword in result][0]
            ...
            async with aiosqlite.connect(constants.Database.name) as database:
                ...
                for url, keyword in rss_links:
                    ...
                    async with database.execute("SELECT id FROM google_alerts_posts WHERE id=?",
                        ...
                        if len(result) == 0:
                            # Try to remove the "<b> </b>" tags from the entry title:
                            try:
                                tags = compile(r"<[^>]+>")
                            ...
                            # If title parsing was successful:
                            else:
                                await database.execute("INSERT INTO google_alerts_posts VALUES (?, ?, ?)",
                                                       (entry_id,
                                                        parsed_title,
