            ...
            async with database.execute("SELECT username FROM twitter_usernames WHERE username=?",
                ...
                # If the username exists in the database:
                    ...
                    async with database.execute("SELECT retweets FROM twitter_usernames WHERE username=?",
                        ...
                        result = [
                            item for state in result for item in state][0]
            ...
            async with database.execute("SELECT username FROM twitter_usernames WHERE username=?",
                ...
                # If the username exists in the database:
                    ...
                    async with database.execute("SELECT retweets FROM twitter_usernames WHERE username=?",
                        ...
                        # Grab the value for the result
                        result = [item for state in result for item in state][0]
    ...
    async def monitor_tweets(self):
        try:
            async with aiosqlite.connect(constants.Database.name) as database:
                ...
                async with database.execute("SELECT * FROM twitter_usernames") as user_cursor:
                    user_list = await user_cursor.fetchall()

                    # Turn the list of tuples into a list:
                    user_list = [user[0] for user in user_list]

                    for username in user_list:
                        # Check the retweet status of the given user:
                        async with database.execute("SELECT retweets FROM twitter_usernames WHERE username=?",
                                                    (username,)) as rt_cursor:
                            rt_status = await rt_cursor.fetchall()

                            # Turn the list of tuples into a list:
                            rt_status = [status[0] for status in rt_status][0]
        ...
        # When the database is first initalized, because there are no usernames,
        # Line 143 throws IndexError. This is meant to catch it and hope till the next loop,
        # that a username is submitted to the database.
        except IndexError:
            pass
